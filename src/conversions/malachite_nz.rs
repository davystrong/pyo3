...
macro_rules! bigint_conversion {
    ($rust_ty: ty, $is_signed: expr, $to_bytes: path) => {
        #[cfg_attr(docsrs, doc(cfg(feature = "malachite-nz")))]
...
impl ToBytesLe for Natural {
    fn to_bytes_le(&self) -> Vec<u8> {
        self.limbs().flat_map(Limb::to_le_bytes).collect()
    }
}

trait ToSignedBytesLe {
    fn to_signed_bytes_le(&self) -> Vec<u8>;
}

impl ToSignedBytesLe for Integer {
    fn to_signed_bytes_le(&self) -> Vec<u8> {
        self.twos_complement_limbs()
            .flat_map(Limb::to_le_bytes)
            .collect()
    }
}

bigint_conversion!(Natural, 0, Natural::to_bytes_le);
bigint_conversion!(Integer, 1, Integer::to_signed_bytes_le);
...
impl<'source> FromPyObject<'source> for Natural {
    fn extract(ob: &'source PyAny) -> PyResult<Natural> {
        ...
        let n_bits = int_n_bits(num)?;
        if n_bits == 0 {
            return Ok(Natural::from(0usize));
        }
        #[cfg(not(Py_LIMITED_API))]
        {
            let buffer = int_to_u32_vec(num, (n_bits + 31) / 32, false)?;
            Ok(Natural::from_limbs_asc(&buffer))
        }
        #[cfg(Py_LIMITED_API)]
        {
            let bytes = int_to_py_bytes(num, (n_bits + 7) / 8, false)?;
            Ok(Natural::from_bytes_le(bytes.as_bytes()))
        }
    }
}
